...
namespace Wells.FrmType
{
    public partial class frmWaitingBox : Form
    {
        ...
        public frmWaitingBox(Form parent ,EventHandler<EventArgs> method,int maxWaitTime,string waitMessage,bool cancelEnable,bool timerVisable)
        {
            maxWaitTime *= 1000;
            Initialize(parent,method, maxWaitTime,waitMessage, cancelEnable, timerVisable);
        }
        public frmWaitingBox(EventHandler<EventArgs> method)
        {
            int maxWaitTime=60*1000;
            string waitMessage = clsWellsLanguage.getString(112);
            bool cancelEnable=true;
            bool timerVisable=true;
            Initialize(null,method, maxWaitTime,waitMessage, cancelEnable, timerVisable);
        }
        ...
        #region Initialize
        private void Initialize(Form parent,EventHandler<EventArgs> method, int maxWaitTime,string waitMessage,bool cancelEnable, bool timerVisable)
        {
            InitializeComponent();
            //initialize form
            this.FormBorderStyle = FormBorderStyle.None;
            this.StartPosition = parent == null ? FormStartPosition.CenterScreen : FormStartPosition.CenterParent;
        ...
        {
            this.Message = clsWellsLanguage.getString(113);
            this.Close();
        }
        ...
        {
            _WaitTime += TimeSpan;
            this.labTimer.Text = string.Format("{0}{1}", _WaitTime / 1000, clsWellsLanguage.getString(114));
            if (!this._AsyncResult.IsCompleted)
            {
                if (_WaitTime > _MaxWaitTime)
                {
                    Message = string.Format("处理数据超时{0}{1}，结束当前操作！", _MaxWaitTime / 1000, clsWellsLanguage.getString(114));
                    timer1.Stop();
                    this.Close();
                }
            }
