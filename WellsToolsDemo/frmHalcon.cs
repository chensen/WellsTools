...
namespace WellsToolsDemo
{
    public partial class frmHalcon : Form
    {
        ...
        {
            InitializeComponent();
            //HOperatorSet.CreateBarCodeModel(new HTuple(), new HTuple(), out barReader);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //HOperatorSet.SetBarCodeParam(barReader, "num_scanlines",);
            //HOperatorSet.SetBarCodeParam(barReader, "element_size_min",);
            //HOperatorSet.SetBarCodeParam(barReader, "element_size_min",);
            //HOperatorSet.SetBarCodeParam(barReader, "element_size_min",);
        }
        ...
        private void copyFile(string path)
        {
            ...
            string[] dirList = Wells.Tools.clsFile.getDirectories(path);
            for (int igg = 0; igg < dirList.Length; igg++)
            {
                string _path = dirList[igg] + "\\resultt";
                if (Directory.Exists(_path))
                {
                    string[] fileList = Wells.Tools.clsFile.getFileNames(_path, "*.jpg", false);
                    for (int jgg = 0; jgg < fileList.Length; jgg++)
                    {
                        string filename = fileList[jgg];
                        if (File.Exists(filename))
                        {
                            HOperatorSet.ReadImage(out obj, filename);
                            HOperatorSet.GetImageSize(obj, out width, out height);
                            x = (int)Math.Ceiling(width.D / w);
                            y = (int)Math.Ceiling(height.D / h);
                        }
                    }
                }
            }
    ...
    public class Snowflake
    {
        ...
        private static long twepoch = 1574155615L; //唯一时间，这是一个避免重复的随机量，自行设定不要大于当前时间戳
